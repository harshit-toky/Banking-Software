...
public class AllLoan extends JFrame implements ActionListener {
    
    JButton ob[]=new JButton[50],back;
    int i=0;
    String pinnumber;
    AllLoan(String pinnumber){
        this.pinnumber=pinnumber;
        setLayout(null);
        setTitle("Applied Loan");
        
        JLabel text = new JLabel("APPLIED LOANS LIST");
        text.setBounds(180,50,500,50);
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        for(j=0;j<i;j++){
            if(ae.getSource() == ob[j]){
                setVisible(false);
                new Display(ob[j].getText()).setVisible(true);
            }
        }
